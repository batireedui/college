...
var TrackballControls = function (object, domElement) {
    ...
    document.addEventListener('click', function(event) {
        ...
        if (clickedElement.tagName.toLowerCase() === 'button') { // зөвхөн button элемент
    ...
    var _state = STATE.NONE,
        ...
        _moveCurr = new Vector2(90, 0),

        _lastAxis = new Vector3(),
        _lastAngle = this.minimumSpeed,
        _isClicked = false,
        ...
        return function rotateCamera() {
            ...
            if (angle) {
                ...
                if(t === 0){
                    ...
                    objectUpDirection.set(0, 1, 0); // Объектын дээд чиглэлийг хадгална
                    objectSidewaysDirection.setLength(_moveCurr.x - _movePrev.x); // Зөвхөн X тэнхлэгийн хөдөлгөөнд утга өгнө
            ...
            } else if (!scope.staticMoving && _lastAngle > scope.minimumSpeed) {
                _lastAngle *= Math.sqrt(1.0 - scope.dynamicDampingFactor);
                _eye.copy(scope.object.position).sub(scope.target);
                quaternion.setFromAxisAngle(_lastAxis, _lastAngle);
                _eye.applyQuaternion(quaternion);
                scope.object.up.applyQuaternion(quaternion);
            } else if (!_isClicked && !this.isPaused) {
                // _lastAngle *= Math.sqrt(1.0 - scope.dynamicDampingFactor);
                _eye.copy(scope.object.position).sub(scope.target);
                quaternion.setFromAxisAngle(_lastAxis, _lastAngle);
                _eye.applyQuaternion(quaternion);
