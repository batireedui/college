...
_select(
    $stmtSuvarga,
    $countSuvarga,
    "SELECT orders.payid, orders.id, UPPER(zovogner), UPPER(tovogner), zutas, tursun, husel, zproducts.price, itemtenger.id, itemtenger.dtuluv, products.name, products.jpname , orders.ognoo FROM `itemtenger` INNER JOIN orders ON itemtenger.orderid = orders.id INNER JOIN zproducts ON itemtenger.zproductid = zproducts.id INNER JOIN products ON itemtenger.productid = products.id  WHERE orders.tuluv = ? and tuluvs = ? ORDER BY orders.id DESC",
    "ii",
    ['1', '0'],
    $payidSuvarga,
    $oidSuvarga,
...
<div class="main-panel">
    <div class="content-wrapper">
        <div class="card">
            <div class="card-body">
                <h4 class="card-title">ТЭНГЭРИЙН СУВАРГА</h4>
                ...
                <div class="row">
                    <div class="col-12">
                        <div class="row">
                            ...
                            <div class="col-sm-2">
                                <input type="submit" class="btn btn-danger" name="suvargahold" value="Архивлах"/>
                            </div>
                            </form>
                        </div>
    ...
    <script>
        ...
        function ExportToExcel(type, fn, dl) {
            ...
            return dl ?
                ...
                XLSX.writeFile(wb, fn || ('Тэнгэрийн суварга-' + dateTime + '.' + (type || 'xlsx')));
        }
