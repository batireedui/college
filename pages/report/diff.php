...
_selectNoParam(
    $cstmt,
    $ccount,
    "SELECT SUM(niit), sum(v4), classid, YEAR(ognoo), MONTH(ognoo), class.sname, class.name FROM `att` INNER JOIN class ON att.classid = class.id GROUP by classid, YEAR(ognoo), MONTH(ognoo);",
    $niit,
    $tas,
    $classid,
    $year,
